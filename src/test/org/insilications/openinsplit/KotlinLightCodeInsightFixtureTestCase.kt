...
//            val extraCompilerArguments = InTextDirectivesUtils.findStringWithPrefixes(extraDirectivesFileText, COMPILER_ARGUMENTS_DIRECTIVE)
...
//            { runCatching { project }.getOrNull()?.let { disableKotlinOfficialCodeStyle(it) } },
    ...
    override fun getProjectDescriptor(): LightProjectDescriptor = getKotlinAndJavaProjectDescriptor()
...
//    }
    ...
    private fun getKotlinAndJavaProjectDescriptor(): LightProjectDescriptor {
        return ProjectDescriptor(LanguageLevel.JDK_21, false)
    }
//
...
//
//            if (minJavaVersion != null && !(InTextDirectivesUtils.isDirectiveDefined(fileText, "RUNTIME") || InTextDirectivesUtils.isDirectiveDefined(
//                    fileText, "WITH_STDLIB"
//                ))
//            ) {
//                error("MIN_JAVA_VERSION so far is supported for RUNTIME/WITH_STDLIB only")
//            }
//            return when {
//                InTextDirectivesUtils.isDirectiveDefined(fileText, "RUNTIME_WITH_SOURCES") -> ProjectDescriptorWithStdlibSources.getInstanceWithStdlibSources()
//
//                InTextDirectivesUtils.isDirectiveDefined(fileText, "RUNTIME_WITHOUT_SOURCES") -> ProjectDescriptorWithStdlibSources.getInstanceNoSources()
//
//                InTextDirectivesUtils.isDirectiveDefined(
//                    fileText, "RUNTIME_WITH_KOTLIN_TEST"
//                ) -> KotlinWithJdkAndRuntimeLightProjectDescriptor.getInstanceWithKotlinTest()
//
//                InTextDirectivesUtils.isDirectiveDefined(
//                    fileText, "RUNTIME_WITH_FULL_JDK"
//                ) -> KotlinWithJdkAndRuntimeLightProjectDescriptor.getInstanceFullJdk()
//
//                InTextDirectivesUtils.isDirectiveDefined(
//                    fileText, "RUNTIME_WITH_JDK_10"
//                ) -> KotlinWithJdkAndRuntimeLightProjectDescriptor.getInstanceWithStdlibJdk10()
//
//                InTextDirectivesUtils.isDirectiveDefined(
//                    fileText, "RUNTIME_WITH_REFLECT"
//                ) -> KotlinWithJdkAndRuntimeLightProjectDescriptor.getInstanceWithReflect()
//
//                InTextDirectivesUtils.isDirectiveDefined(
//                    fileText, "RUNTIME_WITH_SCRIPT_RUNTIME"
//                ) -> KotlinWithJdkAndRuntimeLightProjectDescriptor.getInstanceWithScriptRuntime()
//
//                InTextDirectivesUtils.isDirectiveDefined(
//                    fileText, "RUNTIME_WITH_STDLIB_JDK8"
//                ) -> KotlinWithJdkAndRuntimeLightProjectDescriptor.getInstanceWithStdlibJdk8()
//
//                InTextDirectivesUtils.isDirectiveDefined(fileText, "RUNTIME") || InTextDirectivesUtils.isDirectiveDefined(fileText, "WITH_STDLIB") -> {
...
//                }
//
//                InTextDirectivesUtils.isDirectiveDefined(fileText, "JS_WITH_STDLIB") -> KotlinStdJSWithStdLibProjectDescriptor
//
//                InTextDirectivesUtils.isDirectiveDefined(fileText, "JS_WITH_DOM_API_COMPAT") -> KotlinStdJSWithDomApiCompatProjectDescriptor
//
//                InTextDirectivesUtils.isDirectiveDefined(fileText, "JS") -> KotlinStdJSProjectDescriptor
//
//                InTextDirectivesUtils.isDirectiveDefined(fileText, "ENABLE_MULTIPLATFORM") -> KotlinProjectDescriptorWithFacet.KOTLIN_STABLE_WITH_MULTIPLATFORM
//
//                InTextDirectivesUtils.isDirectiveDefined(fileText, "WITH_COROUTINES") -> KotlinWithJdkAndRuntimeLightProjectDescriptor(
//                    listOf(
//                        kotlinxCoroutines, coroutineContext
//                    ), emptyList()
//                )
//
//                else -> getDefaultProjectDescriptor()
...
//
//    val compilerVersion = InTextDirectivesUtils.findStringWithPrefixes(fileText, "// $KOTLIN_COMPILER_VERSION_DIRECTIVE ")?.let(IdeKotlinVersion.Companion::opt)
//    val languageVersion =
//        InTextDirectivesUtils.findStringWithPrefixes(fileText, "// $LANGUAGE_VERSION_DIRECTIVE ")?.let { LanguageVersion.fromVersionString(it) }
//    val projectLanguageVersion =
//        InTextDirectivesUtils.findStringWithPrefixes(fileText, "// $PROJECT_LANGUAGE_VERSION_DIRECTIVE ")?.let { LanguageVersion.fromVersionString(it) }
//    val apiVersion = InTextDirectivesUtils.findStringWithPrefixes(fileText, "// $API_VERSION_DIRECTIVE ")?.let { ApiVersion.parse(it) }
//
//    InTextDirectivesUtils.findStringWithPrefixes(fileText, "// $COMPILER_PLUGIN_OPTIONS ")?.split("\\s+,\\s+")?.toTypedArray()?.let {
//        KotlinCommonCompilerArgumentsHolder.getInstance(project).update { this.pluginOptions = it }
//    }
//
//    if (compilerVersion != null || languageVersion != null || apiVersion != null || jvmTarget != null || options != null || projectLanguageVersion != null) {
//        configureLanguageAndApiVersion(
//            project,
//            module,
//            compilerVersion ?: languageVersion?.let(IdeKotlinVersion.Companion::fromLanguageVersion) ?: KotlinPluginLayout.standaloneCompilerVersion,
//            languageVersion,
...
//    val registers = InTextDirectivesUtils.findListWithPrefixes(fileText, "// REGISTRY:").map { it.split(' ') }.map { Registry.get(it.first()) to it.last() }
//    try {
...
//    project: Project, configurator: (CodeStyleSettings) -> Unit = { }, body: () -> Unit
//) {
...
fun runAll(
    vararg actions: ThrowableRunnable<Throwable>, suppressedExceptions: List<Throwable> = emptyList()
): Unit = RunAll(actions.toList()).run(suppressedExceptions)
...
//    project: Project, module: Module, languageVersion: LanguageVersion, apiVersion: ApiVersion?, body: () -> Unit
//) {
//    val removeFacet = !module.hasKotlinFacet()
...
//
//        if (removeFacet) {
//            KotlinCommonCompilerArgumentsHolder.getInstance(project).update { this.languageVersion = bundledCompilerVersion.languageVersion.versionString }
//            module.removeKotlinFacet(ProjectDataManager.getInstance().createModifiableModelsProvider(project), commitModel = true)
//        } else {
...
fun Project.allKotlinFiles(): List<KtFile> {
    ...
    return virtualFiles.map { PsiManager.getInstance(this).findFile(it) }.filterIsInstance<KtFile>()
}
...
fun Project.allJavaFiles(): List<PsiJavaFile> {
    ...
    return virtualFiles.map { PsiManager.getInstance(this).findFile(it) }.filterIsInstance<PsiJavaFile>()
}
...
//    return SimpleDataContext.builder().additionalSteps().setParent(parentContext).build()
//}
