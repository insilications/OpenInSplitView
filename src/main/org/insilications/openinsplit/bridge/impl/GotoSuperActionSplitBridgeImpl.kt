package org.insilications.openinsplit.bridge.impl
...
internal class KotlinGoToSuperDeclarationsHandlerSplit : GotoSuperActionSplitBridge {
        ...
        fun gotoSuperDeclarations(targetDeclaration: KtDeclaration): JBPopup? {
            when (val result: HandlerResult? = findSuperDeclarations(targetDeclaration)) {
                is HandlerResult.Single -> {
                    result.item.descriptor?.takeIf { it.canNavigate() }?.navigate(/* requestFocus = */ true)
                }

                is HandlerResult.Multiple -> {
                    val superDeclarationsArray = result.items.mapNotNull { it.declaration.element }.toTypedArray()
    ...
    override fun invoke(project: Project, editor: Editor, file: PsiFile) {
        ...
        LOG.debug { "Invoking KotlinGoToSuperDeclarationsHandlerSplit" }
