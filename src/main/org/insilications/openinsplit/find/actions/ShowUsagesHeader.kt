    ...
    init {
        panel = panel {
            customizeSpacingConfiguration(
                spacingConfiguration = object : IntelliJSpacingConfiguration() {
                ...
                row {
                    ...
                    val titleCell = cell(JLabel(XmlStringUtil.wrapInHtml("<body><nobr>$title</nobr></body>"))).resizableColumn().gap(RightGap.SMALL)
                    titleLabel = titleCell.component

                    processIcon = cell(AsyncProcessIcon("xxx")).gap(RightGap.SMALL).component
